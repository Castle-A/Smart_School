        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                {[
                    ...
                    { label: "Enseignants", value: "84", icon: GraduationCap, color: "text-purple-400", bg: "bg-purple-500/20" },
                        ...
                        {[
                            ...
                            <div key={idx} className="flex items-center justify-between p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center text-sm font-bold text-white border border-white/10">
                                        {teacher.name.charAt(3)}
                                    </div>
                                    <div>
                                        <p className="text-sm font-medium text-white">{teacher.name}</p>
                                        <p className="text-xs text-gray-400">{teacher.subject} • {teacher.classes}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-1">Cahier: {teacher.fillRate}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="flex justify-between text-sm mb-2">
                                <span className="text-gray-300">Validation des Bulletins (T1)</span>
                        ...
                        <div>
                            <div className="flex justify-between text-sm mb-2">
                                <span className="text-gray-300">Saisie des Notes (Trimestre 1)</span>
                ...
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                        {['Tous', 'Primaire', 'Collège', 'Lycée'].map(filter => (
                            <button key={filter} className="px-3 py-1 text-xs rounded-full bg-white/5 hover:bg-white/10 text-gray-300 transition-colors">
                                {filter}
                            </button>
                        ))}
                    </div>
