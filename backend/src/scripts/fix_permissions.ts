...
async function main() {
    ...
    for (const user of schoolUsers) {
        ...
        if (user.rolePermissions.length > 0) {
            console.log(`[SKIP] User ${user.user.email} (${user.role}) already has ${user.rolePermissions.length} permissions.`);
            continue;
        }

        // Skip if role is not one of the administrative roles we track permissions for
        if (!['DIRECTOR', 'SECRETARY', 'SURVEILLANT', 'CENSEUR', 'ACCOUNTANT', 'TEACHER', 'FOUNDER'].includes(user.role)) {
            console.log(`[SKIP] User ${user.user.email} has role ${user.role} which is not targeted.`);
            continue;
        }

        console.log(`[FIX] User ${user.user.email} (${user.role}) has NO permissions. Assigning defaults...`);

        const defaultPermissions = getDefaultPermissionsForRole(user.role as RoleType);
        ...
        if (definitions.length > 0) {
            ...
            console.log(`✅ Assigned ${definitions.length} permissions to ${user.user.email}.`);
        } else {
            console.log(`[ERROR] Could not find definitions for codes: ${defaultCodes.join(', ')}`);
        }
    }
