...
async function main() {
    ...
    const allRelevantUsers = [...users, ...specificUsers].filter((v, i, a) => a.findIndex(t => (t.id === v.id)) === i);

    console.table(allRelevantUsers.map(u => ({
        id: u.id,
        name: `${u.firstName} ${u.lastName}`,
        schoolUsers: u.schoolUsers.map(su => `${su.schoolId} (${su.role})`).join(', ')
    })));
