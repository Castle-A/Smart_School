...
async function main() {
    ...
    const teacher = await prisma.teacher.findFirst({ where: { schoolId, deletedAt: null } });
    if (!teacher) {
        console.log("No teacher found to delete. Skipping delete test.");
    } else {
        console.log(`Requesting deletion for teacher: ${teacher.firstName} ${teacher.lastName} (${teacher.id})`);
        ...
        const found = await prisma.adminRequest.findUnique({ where: { id: request.id } });
        ...
        console.log(`Teacher Status: ${updatedTeacher?.deletedAt ? 'DELETED' : 'ACTIVE'}`);
