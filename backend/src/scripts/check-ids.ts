...
async function main() {
    ...
    const users = await prisma.user.findMany({
        ...
        select: { id: true, firstName: true, lastName: true, email: true, schoolUsers: { select: { schoolId: true, role: true } } }
    });
    console.log('--- USERS (Rog & Molly) ---');
    users.forEach(u => console.log(`${u.firstName} ${u.lastName} (${u.email}) - SchoolUsers: ${JSON.stringify(u.schoolUsers)}`));

    const teachers = await prisma.teacher.findMany({ select: { id: true, firstName: true, lastName: true, schoolId: true, deletedAt: true, userId: true } });
    console.log('--- TEACHERS ---');
    teachers.forEach(t => console.log(`${t.firstName} ${t.lastName} - SchoolId: ${t.schoolId} - DeletedAt: ${t.deletedAt} - UserId: ${t.userId}`));
    ...
    if (mollyPermissions) {
        mollyPermissions.rolePermissions.forEach(rp => console.log(rp.permissionDefinition.code));
    } else {
