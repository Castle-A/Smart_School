...
import {
  BENIN_SUBJECTS_TEMPLATE,
  BeninSubjectTemplate,
} from '../shared/constants/benin-subjects.constants';
...
async function main() {
  ...
  for (const school of schools) {
    ...
    const isMaternelle =
      cycles.includes('PRESCHOOL') || cycles.includes('MATERNELLE');
    const isPrimaire =
      cycles.includes('PRIMARY') || cycles.includes('PRIMAIRE');
    const isCollege =
      cycles.includes('SECONDARY') || cycles.includes('COLLEGE');
    const isLycee = cycles.includes('SECONDARY') || cycles.includes('LYCEE');
    const isTechnique =
      cycles.includes('TECHNICAL') || cycles.includes('TECHNIQUE');
    ...
    console.log(
      `   👉 Identified ${subjectsToInject.length} subjects to inject.`,
    );
    ...
    console.log(
      `   ✅ Added: ${addedCount} | Skipped (Existing): ${skippedCount}`,
    );
  }
...
function mapTemplateCycleToDbCycle(
