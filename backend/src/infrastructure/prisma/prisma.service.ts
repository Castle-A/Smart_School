            ...
            query: {
                $allModels: {
                    async $allOperations({ model, operation, args, query }) {
                        ...
                        const modelMeta = (PrismaService as any).dmmf?.datamodel?.models?.find(
                            (m: any) => m.name === model
                        );
                        const fields = modelMeta?.fields || [];
                        const hasSchoolId = fields.some((f: any) => f.name === 'schoolId');
                        const hasDeletedAt = fields.some((f: any) => f.name === 'deletedAt');

                        const filterOperations = [
                            'findMany', 'findFirst', 'findUnique', 'findUniqueOrThrow',
                            'count', 'update', 'updateMany', 'delete', 'deleteMany', 'aggregate'
                        ];
                        ...
                        if (schoolId && hasSchoolId) {
                            ...
                            if (operation === 'createMany') {
                                if (Array.isArray((args as any).data)) {
                                    (args as any).data.forEach(item => (item['schoolId'] = schoolId));
                                }
                            }
                        }
                        ...
                        if (hasDeletedAt && ['findMany', 'findFirst', 'count', 'aggregate'].includes(operation)) {
                            (args as any).where = (args as any).where || {};
