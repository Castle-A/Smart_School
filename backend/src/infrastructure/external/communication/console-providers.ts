...
import { IEmailProvider, ISendMailOptions } from '../../../application/interfaces/email-provider.interface';
import { ISmsProvider, ISendSmsOptions } from '../../../application/interfaces/sms-provider.interface';
...
export class ConsoleSmsProvider implements ISmsProvider {
    ...
    async sendSms(options: ISendSmsOptions): Promise<void> {
        this.logger.log(`[SMS MOCK] To: ${options.to} | Message: ${options.message}`);
    }
}
