...
export class R2StorageProvider implements StorageAdapter {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (accountId && accessKeyId && secretAccessKey) {
      ...
      this.logger.warn(
        'R2 credentials missing. Storage will fall back to mock/log mode.',
      );
    }
  }
  ...
  async upload(
    ...
    this.logger.log(
      `[R2 Upload Mock] Uploading ${key} (${file.length} bytes) to ${bucket}`,
    );
  ...
  async getFileStream(key: string, bucket: string): Promise<any> {
    this.logger.log(
      `[R2 Stream Mock] Getting stream for ${key} from ${bucket}`,
    );
    throw new Error('Method not implemented in mock provider.');
  }
}
