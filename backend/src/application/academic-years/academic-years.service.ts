    ...
    for (const year of candidates) {
      try {
        ...
        if (validation.canClose) {
          ...
          this.logger.log(`Auto-closed year ${year.name} for school ${year.schoolId}`);
          results.success++;
        } else {
          this.logger.warn(
          ...
          const admins = await this.prisma.schoolUser.findMany({
            ...
            include: {
              user: {
                select: { id: true, email: true, phone: true, firstName: true, directorType: true }
              }
            }
          });
          ...
          for (const admin of admins) {
            ...
            if (admin.role === 'DIRECTOR') {
              ...
              if (u?.directorType === 'MATERNELLE_PRIMAIRE' && !validation.maternellePrimaireCertified) shouldNotify = true;
              if (u?.directorType === 'COLLEGE_LYCEE' && !validation.collegeLyceeCertified) shouldNotify = true;
            ...
            if (!validation.financeCertified && admin.role === 'ACCOUNTANT') shouldNotify = true;
            if (!validation.allStudentsDecided && admin.role === 'CENSEUR') shouldNotify = true;

            if (shouldNotify && u) {
              await this.communication.sendSystemAlert(year.schoolId, u.id, {
                title,
                message,
                email: u.email ?? undefined,
                phone: u.phone ?? undefined,
              }).catch(e => this.logger.error(`Failed multi-channel notify for ${u.id}: ${e.message}`));
            }
          }
        ...
        this.logger.error(`Error processing auto-closure for ${year.name}: ${msg}`);
