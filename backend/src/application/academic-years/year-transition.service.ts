...
export class YearTransitionService {
  ...
  async createNextYear(
    ...
    if (fromYear.status === YearStatus.ARCHIVED) {
      throw new BadRequestException(
        'Impossible de faire un rollover depuis une année archivée',
      );
    }
      ...
      async (tx) => {
        ...
        const newYear = await tx.academicYear.create({
          data: {
            ...
            status:
              dto.startAsDraft || fromYear.status === YearStatus.ACTIVE
                ? YearStatus.DRAFT
                : YearStatus.ACTIVE,
        ...
        if (options.students === StudentStrategy.PROMOTE) {
          ...
          for (const student of students) {
            ...
            const targetSameClassId = student.classId
            ...
            switch (decision.outcome) {
              case 'PASS':
                ...
                if (dec.nextLevel) {
                  const target = createdClasses.find(
                    (c: any) =>
                  ...
                  if (!target) {
                    this.logger.warn(
                      `Smart Promotion: Target class not found for ${student.id} (${dec.nextLevel} ${dec.nextSeries})`,
                    );
                  }
                } else {
