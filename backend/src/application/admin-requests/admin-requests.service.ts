...
export class AdminRequestService {
    ...
    async findMyRequests(schoolId: string, userId: string, includeArchived = false) {
        return this.prisma.adminRequest.findMany({
            where: {
                schoolId,
                requesterId: userId,
    ...
    async resolve(requestId: string, schoolId: string, resolverId: string, status: 'APPROVED' | 'REJECTED', adminComment?: string) {
        ...
        if (request) {
            const title = status === 'APPROVED' ? 'Requête Approuvée' : 'Requête Refusée';
        ...
        switch (type) {
            ...
            case 'UPDATE_TEACHER':
                ...
                Object.keys(scalarData).forEach(key => {
                    ...
                    if (key === 'hireDate') {
                        if (newValue) newValue = new Date(newValue).toISOString().split('T')[0];
                        if (oldValue) oldValue = new Date(oldValue).toISOString().split('T')[0];
                    }
                ...
                if (newSubjects && Array.isArray(newSubjects)) {
                    const currentSubjectNames = currentTeacher.subjects.map((s: any) => s.name).sort();
                    const newSubjectNames = [...newSubjects].sort();

                    const isSubjectsDifferent = JSON.stringify(currentSubjectNames) !== JSON.stringify(newSubjectNames);
                    ...
                    if (isSubjectsDifferent) {
                        ...
                        newSubjects.forEach((name: string) => {
                            const match = foundSubjects.find(s => s.name === name);
            ...
            case 'CLASS_ASSEMBLY':
                ...
                for (const assign of assignments) {
                    const su = schoolUsers.find((u: any) => u.userId === assign.teacherUserId);
                    if (!su) continue;
