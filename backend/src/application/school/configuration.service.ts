...
export class ConfigurationService {
  ...
  async getConfig(schoolId: string) {
    let config = await this.prisma.schoolConfig.findUnique({
      where: { schoolId },
    });

    // Initialisation automatique avec les valeurs par défaut du schéma si manquante
    if (!config) {
      config = await this.prisma.schoolConfig.create({
        data: { schoolId },
      });
    }

    // TRANSFORMATION: Remplacer le logo (URL publique cassée) par l'URL signée
    return {
      ...config,
      logo: await this.storageService.getSignedSchoolLogoUrl(schoolId),
    };
  }
  ...
  async getSetting<T>(
    schoolId: string,
    key: keyof UpdateSchoolConfigDto,
    defaultValue: T,
  ): Promise<T> {
