...
export class AcademicCalendarService {
  ...
  async create(
    ...
    const isSystemType = ['ACADEMIC_PERIOD', 'HOLIDAY', 'EXAM'].includes(
      dto.type,
    );

    if (isSystemType && !['DIRECTOR', 'FOUNDER'].includes(userRole)) {
      throw new ForbiddenException(
        "Seuls le Directeur et le Fondateur peuvent définir la structure de l'année scolaire (Vacances, Examens).",
      );
    }
    ...
    if (userRole === 'ACCOUNTANT' && dto.type !== 'FINANCIAL') {
      throw new ForbiddenException(
        'Le comptable ne peut ajouter que des événements financiers.',
      );
    }
  ...
  async findAll(schoolId: string, userRole: string = 'ADMIN') {
    ...
    if (
      !['DIRECTOR', 'FOUNDER', 'CENSOR', 'ACCOUNTANT', 'SECRETARY'].includes(
        userRole,
      )
    ) {
  ...
  async update(
    ...
    if (event.isSystem && !['DIRECTOR', 'FOUNDER'].includes(userRole)) {
      throw new ForbiddenException(
        'Seul le Fondateur/Directeur peut modifier cet événement système.',
      );
    }
  ...
  async delete(id: string, schoolId: string, userRole: string) {
    ...
    if (event.isSystem && !['DIRECTOR', 'FOUNDER'].includes(userRole)) {
      throw new ForbiddenException(
        'Impossible de supprimer un événement système.',
      );
    }
