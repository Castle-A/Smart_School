...
export class StudentsService {
  ...
  async create(
    ...
    const isDirectorOrFounder = ['DIRECTOR', 'FOUNDER', 'ADMIN'].includes(
      userRole,
    );
    ...
    const result = await this.prisma.$transaction(async (prisma) => {
      ...
      const student = await prisma.student.create({
        data: {
          ...
          schoolId,
          categoryId: createStudentDto.categoryId,
          status,
        },
      });
      ...
      if (createStudentDto.payment) {
        const { registrationAmount, tuitionAmount, method } =
  ...
  async reEnroll(studentId: string, schoolId: string, dto: any) {
    // 1. Verify existence
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updatedStudent = await tx.student.update({
        ...
        data: {
          ...
          status: 'ACTIVE', // Reactivate if was archived/pending
        },
      });

      // Create Payment if provided
      if (dto.payment && dto.payment.registrationAmount + dto.payment.tuitionAmount > 0) {
        if (dto.payment.registrationAmount > 0) {
          await tx.payment.create({
            data: {
              studentId,
              schoolId,
              amount: dto.payment.registrationAmount,
              reason: "Frais de Réinscription",
              method: dto.payment.method,
              date: new Date(),
            }
          });
        }
        if (dto.payment.tuitionAmount > 0) {
          await tx.payment.create({
            data: {
              studentId,
              schoolId,
              amount: dto.payment.tuitionAmount,
              reason: "Scolarité (Acompte Réinscription)",
              method: dto.payment.method,
              date: new Date(),
            }
          });
