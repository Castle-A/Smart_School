...
export class TransitionsService {
  ...
  async getDecisions(schoolId: string, classId: string, academicYear: string) {
    return this.prisma.studentAcademicHistory.findMany({
      where: {
        classId,
        academicYear,
        student: { schoolId }, // Verify school ownership via relation
      },
      include: {
        student: {
          select: {
  ...
  async getClosureStatus(schoolId: string) {
    ...
    const pendingDecisionsCount = studentIds.filter(
      (id) => !studentsWithDecision.has(id),
    ).length;
  ...
  async closeYear(schoolId: string) {
    ...
    if (
      !school.activeYear.financeCertified ||
      !school.activeYear.maternellePrimaireCertified ||
      !school.activeYear.collegeLyceeCertified
    ) {
      throw new Error(
        'Impossible de clôturer : toutes les certifications (Finance, Maternelle/Primaire, Collège/Lycée) ne sont pas validées.',
      );
    }
