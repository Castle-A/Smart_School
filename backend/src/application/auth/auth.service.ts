...
export class AuthService {
  ...
  async login(user: any) {
    ...
    // Validation: Warn if permissions array is empty for non-TEACHER roles
    if (
      (!payload.permissions || payload.permissions.length === 0) &&
      normalizedRole !== 'TEACHER'
    ) {
      this.logger.warn(
        `User ${user.email} (${normalizedRole}) has no permissions assigned`,
        'AuthService',
      );
    }
    ...
    return {
      ...
      user: {
        ...
        supportLevel: (user as any).supportLevel,
  ...
  async registerFounder(data: {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      // 5. Création d'une configuration par défaut pour l'école
