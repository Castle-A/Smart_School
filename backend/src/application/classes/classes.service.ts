...
export class ClassesService {
  ...
  async initializeDefaults(schoolId: string, userId: string) {
    ...
    const levels: {
      name: string;
      cycle: 'PRESCHOOL' | 'PRIMARY' | 'JUNIOR_HIGH' | 'SENIOR_HIGH';
    }[] = [];

    const isPrimary = ['PRIMARY_PRESCHOOL', 'BOTH'].includes(
      schoolUser.directorType || 'BOTH',
    );
    const isCollege = ['COLLEGE', 'BOTH'].includes(
      schoolUser.directorType || 'BOTH',
    );
