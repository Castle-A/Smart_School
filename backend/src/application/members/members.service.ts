...
export class MembersService {
    ...
    if (['CENSOR', 'SUPERVISOR', 'ACCOUNTANT'].includes(role)) {
      ...
      if (count > 0) {
        throw new ConflictException(
          `Un seul membre avec le rôle ${role} est autorisé par école.`,
        );
      }
    }
    ...
    if (['DIRECTOR', 'SECRETARY'].includes(role)) {
      ...
      if (existingUsers.length > 0) {
        ...
        if (directorType === 'BOTH') {
          throw new ConflictException(
            `Un membre de type BOTH ne peut être créé s'il existe déjà d'autres membres de ce rôle.`,
          );
        }
        ...
        if (hasBoth) {
          throw new ConflictException(
            `Impossible de créer ce membre car un membre de type BOTH existe déjà.`,
          );
        }

        const hasSameType = existingUsers.some(
          (u) => u.directorType === directorType,
        );
        if (hasSameType) {
          throw new ConflictException(
            `Un membre de type ${directorType} existe déjà pour ce rôle.`,
          );
        }
        ...
        if (existingUsers.length >= 2) {
          throw new ConflictException(
            `Le nombre maximum de membres pour ce rôle est atteint (un par cycle).`,
          );
        }
      }
    }
  ...
  async createMember(dto: CreateMemberDto, tempPassword: string) {
    ...
    if (dto.role === 'DIRECTOR' && !dto.directorType) {
      throw new BadRequestException(
        'Le type de directeur est requis pour le rôle DIRECTEUR',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        'Type de directeur invalide pour le secrétaire',
      );
    }

    // Validation des contraintes de rôle (Unicité, Cycles)
    await this.validateRoleConstraints(
      dto.schoolId,
      dto.role,
      dto.directorType,
    );
  ...
  async updateMemberPermissions(
    ...
    if (
      directorType &&
      !['PRIMARY_PRESCHOOL', 'COLLEGE', 'BOTH'].includes(directorType)
    ) {
