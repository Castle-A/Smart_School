...
export class AnalyticsService {
    ...
    async getSchoolKPIs(schoolId: string) {
        console.log(`[Analytics] Debugging KPIs for schoolId: ${schoolId}`);
        ...
        const absenteeismRate = studentCount > 0 ? ((todaysAttendance / studentCount) * 100).toFixed(1) : 0;

        console.log(`[Analytics] KPIs Found: Students=${studentCount}, Teachers=${teacherCount}, Classes=${activeClasses}, Revenue=${payments._sum.amount}`);
    ...
    async debugDump() {
        ...
        for (const school of schools) {
            const students = await this.prisma.student.count({ where: { schoolId: school.id } });
            const teachers = await this.prisma.teacher.count({ where: { schoolId: school.id } });
